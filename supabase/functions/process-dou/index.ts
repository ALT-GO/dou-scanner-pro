...
const STATE_ORGANS = [
  {
    pattern:
      /\b(Governo do Distrito Federal|GDF|TCDF|CLDF|MPDFT|PGDF|SES[\-\/]DF|SEE[\-\/]DF|NOVACAP|CAESB|CEB|TERRACAP|METRÔ[\-\/]DF)\b/gi,
    state: "DF",
  },
  {
    pattern:
      /\b(Governo do Estado de São Paulo|SABESP|CPTM|CDHU|DERSA|DER[\-\/]SP|ARTESP|IMESP|PRODESP|CPOS|FDE|IPT|UNICAMP|UNESP|USP)\b/gi,
    state: "SP",
  },
  {
    pattern:
      /\b(Governo do Estado de Minas Gerais|CEMIG|COPASA|DER[\-\/]MG|DEOP[\-\/]MG|CODEMIG|UFMG|UFJF|UFU|UFLA|UFV)\b/gi,
    state: "MG",
  },
];
...
function detectState(text: string): string | null {
  ...
  const stateFullMap: [RegExp, string][] = [
    [/\bem\s+Sergipe\b/gi, "SE"],
    [/\bem\s+Alagoas\b/gi, "AL"],
    [/\bno\s+Rio\s+de\s+Janeiro\b/gi, "RJ"],
    [/\bna\s+Bahia\b/gi, "BA"],
    [/\bno\s+Paran[áa]\b/gi, "PR"],
    [/\bno\s+Rio\s+Grande\s+do\s+Sul\b/gi, "RS"],
    [/\bem\s+Santa\s+Catarina\b/gi, "SC"],
    [/\bem\s+Goi[áa]s\b/gi, "GO"],
    [/\bem\s+Pernambuco\b/gi, "PE"],
    [/\bno\s+Cear[áa]\b/gi, "CE"],
    [/\bno\s+Par[áa]\b/gi, "PA"],
    [/\bno\s+Maranh[ãa]o\b/gi, "MA"],
    [/\bno\s+Mato\s+Grosso\b/gi, "MT"],
    [/\bno\s+Mato\s+Grosso\s+do\s+Sul\b/gi, "MS"],
    [/\bno\s+Esp[ií]rito\s+Santo\b/gi, "ES"],
    [/\bna\s+Para[ií]ba\b/gi, "PB"],
    [/\bno\s+Rio\s+Grande\s+do\s+Norte\b/gi, "RN"],
    [/\bno\s+Piau[ií]\b/gi, "PI"],
    [/\bem\s+Tocantins\b/gi, "TO"],
    [/\bem\s+Rond[ôo]nia\b/gi, "RO"],
    [/\bno\s+Acre\b/gi, "AC"],
    [/\bno\s+Amap[áa]\b/gi, "AP"],
    [/\bno\s+Amazonas\b/gi, "AM"],
    [/\bem\s+Roraima\b/gi, "RR"],
    [/\bem\s+São\s+Paulo\b/gi, "SP"],
    [/\bem\s+Minas\s+Gerais\b/gi, "MG"],
  ...
  const genericUf =
    /\b[A-ZÀ-Ú][A-ZÀ-Úa-zà-ú\s]+\s*[-–—\/]\s*(AC|AL|AP|AM|BA|CE|DF|ES|GO|MA|MT|MS|MG|PA|PB|PR|PE|PI|RJ|RN|RS|RO|RR|SC|SP|SE|TO)\b/g;
...
async function getCompanyContext(SUPABASE_URL: string, SUPABASE_SERVICE_ROLE_KEY: string): Promise<string> {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/company_capabilities?select=descricao`, {
    headers: {
...
async function runProcessingJob(
  ...
  const companyContext = await getCompanyContext(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);
  ...
  console.log(
    `Pre-filter: ${stats.total} blocks → ${stats.competitors} competitors, ${stats.technical} technical, ${stats.discarded} discarded`,
  );
                ...
                contents: [
                  {
                    ...
                    parts: [
                      {
                        text: `Analise este array JSON de blocos e extraia as publicações válidas:
...
${batchJson}`,
                              ...
                              items: {
                                ...
                                properties: {
                                  ...
                                  estado_execucao: {
                                    type: "STRING",
                                    description:
                                      "UF de 2 letras onde o serviço/obra será executado (ex: SP, BA, PR, MG, DF). Baseie-se APENAS no local de execução, NUNCA em siglas no nome do órgão.",
                                  },
          ...
          if (!response.ok) {
            ...
            console.error(
              `Gemini API error (batch ${batchIndex + 1}, attempt ${attempt + 1}): ${response.status} ${errorText}`,
            );
    ...
    if (failedBatches > 0) {
      console.log(
        `${failedBatches}/${batches.length} batches failed — proceeding with ${batches.length - failedBatches} successful`,
      );
    }
    ...
    console.log(
      `Reconstructed full_text for ${aiPublications.length} publications from block IDs (section derived from estado_execucao in code)`,
    );
  }
  ...
  const opportunities = publications.filter((p: any) =>
    ["SP", "MG", "DF", "AVISOS_DIVERSOS"].includes(p.section),
  ).length;
...
serve(async (req) => {
  ...
  try {
    ...
    const processJob = async () => {
      ...
      const result = await runProcessingJob(text, readingId, GEMINI_API_KEY, SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);
    ...
    if (!edgeRuntime?.waitUntil) {
      ...
      return new Response(JSON.stringify({ ...result, completedSynchronously: true, readingId }), {
    ...
    return new Response(JSON.stringify({ success: true, accepted: true, readingId }), {
      status: 202,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
